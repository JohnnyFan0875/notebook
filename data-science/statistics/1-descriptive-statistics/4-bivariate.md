...
| Categorical        | Categorical        | Cross-tabulation (covered in Section 2)  |
| Numerical          | Categorical        | Group comparison, boxplot by group       |
| Multiple Numerical | Multiple Numerical | Correlation matrix, heatmap              |

> 📌 **Correlation ≠ Causation**. A strong correlation tells you two variables move together — it does **not** tell you that one causes the other. 相關不代表因果，這是統計中最常被誤解的概念。

---

## 4.1 Numerical × Numerical: Correlation
...
> ⚠️ If these assumptions are violated, use **Spearman ρ** instead.
...
> 💡 **When in doubt**, use Spearman ρ — it makes fewer assumptions and is almost as efficient as Pearson when the data is normal. 不確定時，Spearman 是比較保守安全的選擇。
...
> ⚠️ **Anscombe's Quartet warning**: Four datasets can have almost identical correlations but look completely different visually. Always plot first.  
> 安斯康姆四重奏：四個相關係數幾乎相同的資料集，視覺上卻截然不同。先畫圖，再計算。

---

## 4.2 Numerical × Categorical: Group Comparison
...
## 4.3 Multiple Numerical Variables: Correlation Matrix
...
> - White/light → near zero (no linear relationship)
