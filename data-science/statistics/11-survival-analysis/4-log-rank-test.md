...
> 📌 **Log-rank test 的定位**：它是 KM 曲線的配套假設檢定，就像 t-test 是比較兩組平均數的工具。視覺上兩條 KM 曲線看起來不同，不等於統計上顯著不同。Log-rank test 給出正式的 p 值。
...
> 💡 The log-rank test gives **equal weight to all event times**. This makes it most sensitive to detecting differences when the **hazard ratio is constant** across time (proportional hazards). When hazards cross or the difference only appears late, the log-rank test can miss it — see Section 4.5 for alternatives.
...
> ⚠️ A significant multivariate log-rank result only tells you that **at least one group differs**. It does not tell you *which* groups differ from each other. Use pairwise log-rank tests with Bonferroni or Holm correction for post-hoc comparisons.  
> 多組 log-rank test 只告訴你「至少有一組不同」，不告訴你是哪幾組之間有差異。事後比較需要校正多重檢定。
...
## 4.5 Weighted Log-Rank Variants
...
> ⚠️ The log-rank test answers only "are these groups different?" — not "how different?" and not "why?". For quantification and covariate adjustment, move to the Cox model (Section 5).
