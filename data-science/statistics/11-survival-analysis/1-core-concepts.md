...
> 📌 **最常見的錯誤**：把存活分析的資料當成一般二元結果來分析，完全忽略「時間」維度和設限（censoring）觀測值。這樣的分析不只不精確，而且在方向上就是錯的。

---

## 1.1 The Two Core Variables

Every survival analysis dataset requires exactly two pieces of information per observation:

| Variable          | 中文       | Description                                                     | Example                          |
| ----------------- | ---------- | --------------------------------------------------------------- | -------------------------------- |
| **Time (T)**      | 存活時間   | Duration from study entry to the event or censoring             | 14.5 months from diagnosis       |
| **Event (δ)**     | 事件指標   | Binary: 1 if the event was observed, 0 if censored              | 1 = died, 0 = still alive at end |

This **(T, δ)** pair is the fundamental unit of survival data.

> 💡 "Survival" is a historical term — the event does not have to be death. It can be any well-defined, non-repeating event: churn, failure, default, conversion, relapse. The methods work identically regardless of what the event is.  
> 「存活」只是歷史術語，事件本身可以是客戶流失、設備故障、貸款違約等任何單次發生的事件。

---

## 1.2 What Is Censoring?
...
| ----------------------- | ------------ | ------------------------------------------------------------------- | ---------------------------------------------------- |
| **Right censoring**     | 右設限       | Event has not occurred by the end of observation                    | Patient still alive when study ends (most common)    |
| **Left censoring**      | 左設限       | Event occurred before observation began — exact time unknown        | Infection already present at first blood test        |
...
| subject_id | time | event | treatment | age |
| ---------- | ---- | ----- | --------- | --- |
| 1          | 10   | 1     | A         | 55  |
| 2          | 20   | 0     | A         | 62  |
| 3          | 14   | 0     | B         | 48  |
...
## 1.4 Simulating Realistic Survival Data
...
## 1.5 Common Data Errors to Avoid
