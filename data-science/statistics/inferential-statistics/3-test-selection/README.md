...
Use this single-page guide to choose an appropriate statistical test based on your **outcome type**, **study design**, and **assumptions**. It links back to the relevant sections in this chapter so you can jump straight to details.

## 1. Quick Decision Rules

1. **Start from the outcome (dependent variable)**

   - **Continuous/approximately normal** → parametric tests (t/ANOVA) if assumptions hold
   - **Ordinal / skewed / outliers / small n** → non-parametric (rank-based)
   - **Binary / counts / proportions** → exact/binomial/χ²/Fisher

2. **Check dependence**

   - **Independent groups** vs **paired/repeated** → drives test family

3. **Check assumptions** (see `../2-hypothesis-testing-framework/assumption-tests/`)

   - **Normality** (Shapiro–Wilk) and **equal variance** (Levene/Brown–Forsythe)
   - If violated → non-parametric or robust alternatives (Welch, rank tests)

4. **Number of groups / factors** matters

   - 2 groups vs ≥3 groups; one factor vs two factors (interaction → two-way ANOVA)

## 2. Fast Lookup Table

### 2.1 Continuous or Ordinal Data

Used when the dependent variable is **numeric (interval/ratio)** or **ordinal (ordered category)**.

#### 2.1.1 Parametric vs Non-parametric Tests
...
- [**Normality**](../2-hypothesis-testing-framework/assumption-tests/normality-tests.md): Shapiro–Wilk; also inspect hist/Q–Q plots
- [**Equal variance**](../2-hypothesis-testing-framework/assumption-tests/variance-tests.md): Levene / Brown–Forsythe
- [**Independence**](../2-hypothesis-testing-framework/assumption-tests/README.md): by design; for serial data use Durbin–Watson / runs test

If any assumption fails, choose the corresponding **robust** or **non-parametric** alternative from the table above.

## 4. Report What Matters

- Test name and rationale (why this test)
- [Effect size](../4-effect-size-and-power/effect-size.md) (e.g., **Cohen’s d**, **η² / η²ₚ**, **odds ratio**)
- [**Confidence intervals**](../1-sampling-and-distributions/confidence-interval.md), not just p-values
- Multiple comparisons control when doing many pairwise tests (Tukey, Bonferroni, FDR)
- Power or sample size planning when feasible → `../4-effect-size-and-power/power-analysis.md`
