...
Key point: Core Principle: Check assumptions first, then run the test. When assumptions are violated, data transformation (log, Box–Cox) should be given priority, or the motherless method should be used instead.
...
Tip: Practical order of checking: Independence → Normality → Equal Variance. Independence is a design issue — if violated, the other checks are moot.
...
Warning: Large sample caution: For n > 200, Shapiro–Wilk will almost always reject normality even for trivially small deviations. With large n, CLT usually applies — prioritize visual checks over p-values.
...
## Variance Tests (Homoscedasticity)

Equal variance (homoscedasticity) means variance is constant across groups. Required by Student's t-test and one-way ANOVA.

Key point: This step is also called a homogeneity of variance check. If variability differs too much across groups, the p-values from the classic t-test or ANOVA can become unreliable.
...
Tip: Default rule: Always use t. It is always valid and converges to z for large n.
