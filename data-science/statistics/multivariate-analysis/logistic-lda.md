...
Key point: Why linear regression cannot be used directly for classification: Linear regression can predict values ​​exceeding 1 or less than 0, which is meaningless for probability. Furthermore, linear regression assumes a normal distribution of errors, whereas the binary outcome (0/1) clearly violates this assumption. Logistic regression uses the sigmoid function to compress the linear prediction value into the [0, 1] interval, perfectly solving this problem.
...
Tip: AUC interpretation: AUC = 0.5 means no better than random; AUC = 1.0 means perfect discrimination. AUC can be interpreted as the probability that the model ranks a randomly chosen positive case higher than a randomly chosen negative case.
...
## Logistic vs Linear Regression vs LDA — Decision Guide
...
## Logistic Regression vs. LDA Intuition

| Method | Models what directly? |
| ------ | --------------------- |
| **Logistic regression** | `P(Y|X)` |
| **LDA** | `P(X|Y)` plus class priors |

Tip: Logistic regression is usually more robust under weaker assumptions. LDA can be excellent when class distributions are close to Gaussian with similar covariance structure.
