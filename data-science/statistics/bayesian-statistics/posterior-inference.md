...
Key point: The posterior distribution is Bayesian inference. Full answer: Don't stop at just reporting the posterior mean. The shape, width, and tails of the posterior distribution contain important information. One number doesn’t tell the whole story.

## Point Estimates from the Posterior

Three common ways to summarize the posterior with a single number:

| Estimate | Formula / Definition | Best Used When |
| -------------------------- | ---------------------------------- | -------------------------------------- |
| **Posterior Mean** | E[θ \ | D] = ∫ θ · P(θ\ | D) dθ | Symmetric, well-behaved posteriors |
| **Posterior Median** | 50th percentile of posterior | Skewed posteriors; robust choice |
| **MAP (Maximum A Posteriori)** | argmax_θ P(θ \ | D) | When a single "most likely" value needed |

Tip: Posterior Mean vs MAP: The posterior mean minimizes expected squared error; MAP minimizes expected 0-1 loss. For symmetric distributions, they're equal. For skewed distributions, the mean is typically preferred as a summary. Posterior mean = minimizes expected squared error; MAP = posterior mode. It is the same when the distribution is symmetrical, but pay attention to the difference when it is skewed.
...
Tip: Key difference from a frequentist confidence interval: - Confidence interval: "If I repeated this experiment many times, 95% of such intervals would contain the true θ." (θ is fixed; the interval is random.) - Credible interval: "Given the data I observed, there is a 95% probability that θ lies in this interval." (θ is random; the interval is computed once.) The correct interpretation of confidence intervals is very counterintuitive. The interpretation of the confidence interval is what most people intuitively think the confidence interval should mean.
...
Tip: For skewed posteriors, the HDI will be narrower and shifted toward the high-density region. For symmetric posteriors, ETI ≈ HDI.
...
Tip: The posterior predictive accounts for two sources of uncertainty: (1) uncertainty about the parameter θ, and (2) inherent randomness even if θ were known. This is why predictions are always more uncertain than the posterior alone suggests. The posterior predictive distribution takes into account both "uncertainty about the parameter" and "randomness of the data itself", and is therefore wider than the posterior itself.
...
✅ If the observed statistic falls comfortably within the simulated distribution — model is plausible. Warning: If the observed statistic is in the extreme tails — model may be misspecified.

## Summary: Posterior Inference Toolkit
...
## A Good Bayesian Reporting Habit

Try to report:

1. a central estimate
2. a credible interval
3. a probability statement relevant to the decision
4. a posterior predictive implication if future outcomes matter

Tip: Bayesian reporting is strongest when it reads like an uncertainty statement rather than a frequentist report with different notation.
