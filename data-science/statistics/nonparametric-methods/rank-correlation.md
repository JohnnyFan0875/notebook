...
Key point: Parametric equivalent: Pearson r (covered in Descriptive Statistics – Bivariate Analysis) Key point: Key difference: Pearson measures *linear* relationships; rank correlations measure *monotonic* relationships — whether one variable consistently increases or decreases as the other does, regardless of the rate.
...
Tip: τ values are typically smaller than ρ for the same dataset. This doesn't mean it's weaker — the scales are simply different. A τ of 0.6 is often considered comparable to a ρ of 0.75+.
...
## Pearson vs Spearman vs Kendall: When to Use Which
...
Tip: General recommendation: - Default: Spearman ρ — widely used, well-understood, good for most situations - Small samples or many ties: Kendall τ — more robust, better statistical properties - Normal continuous data, linear relationship confirmed by scatter plot: Pearson r
...
## Visualization: Scatter Plot with Rank Overlays
...
## Monotonic vs Linear: Why It Matters
...
Tip: Key insight: For a linear relationship, Pearson r ≈ Spearman ρ. For a non-linear monotonic relationship (like exponential growth), Pearson r underestimates the true monotonic strength while Spearman ρ captures it well.
...
If you need a quick default, Spearman is often fine. If the sample is small or the data contains many tied ranks, Kendall's tau is often more stable and easier to interpret probabilistically.
