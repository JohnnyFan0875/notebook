...
> 📌 **季節性是時間序列中最常見的模式之一**：如果你的資料有明顯的週期性重複，卻沒有用季節性模型，殘差裡就會留有大量可被利用的結構。SARIMA 是處理季節性時間序列最重要的古典方法。

---

## 6.1 Identifying Seasonality
...
> 💡 The subseries plot shows the **within-year pattern** (which months are high/low) as well as the **year-over-year trend** (is the pattern growing?). If each month's lines trend upward together, you likely need a multiplicative model.

---

## 6.2 SARIMA: Adding Seasonal Terms
...
> 💡 Don't be intimidated by the notation. In practice: fit SARIMA(p,d,q)(P,D,Q)[m] where m is your seasonal period, and let the ACF/PACF at seasonal lags guide P and Q.

---

## 6.3 Seasonal ACF/PACF: Reading the Seasonal Structure
...
## 6.4 Fitting SARIMA with statsmodels
...
## 6.5 Automatic Seasonal Order Selection
...
## 6.7 Alternative: STL + ARIMA (STL Forecasting)
...
> 💡 **STL + ARIMA vs SARIMA**: STL handles non-linear and evolving seasonal patterns better than SARIMA's rigid multiplicative/additive structure. For complex seasonality, STL + ARIMA often outperforms SARIMA.

---

## 6.8 Handling Multiple Seasonalities: Prophet
