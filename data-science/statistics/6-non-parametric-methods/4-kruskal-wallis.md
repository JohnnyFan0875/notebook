# 4. Multiple Groups: Kruskal-Wallis Test
...
> 📌 **Think of it as**: An extension of Mann-Whitney U to three or more groups — all based on ranks.

---

## 4.1 When to Use Kruskal-Wallis
...
> ⚠️ Like Mann-Whitney U, Kruskal-Wallis does **not** assume normality, but it does assume that the groups have similar **shapes** (just possibly different locations) when interpreting results as a median comparison.
...
> 💡 A significant result tells you *something* is different — but not *which* groups differ. Post-hoc tests are needed to pinpoint where the differences lie.
...
## 4.5 Post-hoc Tests: Dunn's Test

When Kruskal-Wallis is significant, Dunn's Test performs **pairwise comparisons** with correction for multiple testing.

> ⚠️ **Why correction?** Performing multiple comparisons inflates the Type I error rate. If α = 0.05 and you make 3 comparisons, the probability of at least one false positive is much higher than 5%.
...
> 💡 **Recommended default**: Use `p_adjust='holm'` for most practical situations — it's less conservative than Bonferroni while still controlling Type I error.
...
## 4.7 Kruskal-Wallis vs One-Way ANOVA
