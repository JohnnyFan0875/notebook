...
> 📌 **這一章可能是整個統計學最重要的一章**  
> Central Limit Theorem (CLT) 解釋了為什麼 t-test、ANOVA、回歸分析等幾乎所有參數方法都能運作。沒有 CLT，inferential statistics 就沒有基礎。

---

## 5.1 The Core Problem: Sample vs Population
...
> 💡 **Sampling error is not a mistake** — it's the natural variability that occurs because you're working with a subset of the population. Different samples give different x̄ values. The key question is: how are these x̄ values distributed?
...
> 💡 **Key observation**: Even though the population is skewed, the distribution of sample means looks approximately Normal. **This is the Central Limit Theorem.**

---

## 5.3 Central Limit Theorem (中央極限定理)
...
> 💡 **The "n ≥ 30 rule"** is a rough guideline, not a law. The more non-normal your population, the larger your sample needs to be.  
> n ≥ 30 是常見的經驗法則，但不是絕對的。母體越偏離常態，需要的樣本數越大。
...
## 5.4 Standard Error (SE) Revisited

The **Standard Error** is the standard deviation of the sampling distribution of x̄:

$$SE = \frac{\sigma}{\sqrt{n}}$$

> ⚠️ **SD vs SE reminder (from Section 3.3.4)**:
> - **SD** describes variability of individual data points
> - **SE** describes variability of sample means — how precise is your estimate of μ?
...
> 💡 To halve the SE, you need to **quadruple** the sample size (because of the √n relationship). This has major implications for study design and cost.  
> SE 減半需要 4 倍的樣本數，這是研究設計中非常重要的實務考量。

---

## 5.5 When σ is Unknown: Using the t-Distribution
...
## 5.6 Sampling Distribution for Proportions
...
## 5.7 Why This Matters: The Bridge to Inference
...
> 💡 **The key insight**: Even if your raw data is not normally distributed, the **means** of large enough samples are approximately normal. This is what allows us to use normal-based tests on real-world data that is never perfectly normal.  
> 即使原始資料不是常態分佈，只要樣本夠大，樣本平均數就近似常態分佈。這是幾乎所有參數方法的正當性基礎。
