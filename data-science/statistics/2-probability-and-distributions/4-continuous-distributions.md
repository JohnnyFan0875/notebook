...
> 📌 **為什麼連續分佈這麼重要？**  
> Normal distribution 是統計學的核心。t、Chi-square、F 分佈都從它衍生出來，而這些分佈正是 t-test、卡方檢定、ANOVA 等假設檢定的基礎。學好這一章，Inferential Statistics 會清晰很多。
...
### Standard Normal Distribution (標準常態分佈)
...
> 💡 **Z-score 的意義**：Z-score 告訴你某個值距離平均數幾個標準差。Z = 2 代表比平均高 2 個標準差。這讓不同尺度的資料可以互相比較。
...
> 💡 **直覺**：小樣本估計母體時，不確定性更高，所以尾巴要更厚來反映這個不確定性。隨著樣本數增加（df 增加），t 分佈會越來越接近 Normal 分佈。
...
## 4.3 Chi-square Distribution (χ² 分佈)
...
## 4.5 Exponential Distribution (指數分佈)
...
> 💡 **If Poisson counts events per unit time, Exponential models the waiting time until the next event.**  
> Poisson 計算單位時間內的事件數，Exponential 計算兩個事件之間的等待時間。
...
> 💡 If a lightbulb has been working for 1,000 hours, the probability of it lasting another 500 hours is the same as a brand-new bulb lasting 500 hours. Past survival time gives no information about future lifetime.

---

## 4.6 Uniform Distribution (均勻分佈)
...
## 4.7 Comparing All Continuous Distributions
...
> 💡 These relationships matter in practice: when you run a t-test, it uses the t-distribution. When you run ANOVA, it uses the F-distribution. When you run a Chi-square test, it uses the χ² distribution. Understanding where these distributions come from makes the tests much more intuitive.
