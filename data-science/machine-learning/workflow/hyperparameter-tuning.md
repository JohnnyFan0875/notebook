...
Hyperparameter tuning is the process of finding the **optimal set of hyperparameters** that leads to the best model performance. Unlike model parameters (learned during training), hyperparameters are set before training and control aspects such as [regularization](../foundations/regularization.md) strength, learning rate, or tree depth.

⚠️ **Critical Note:** Always perform hyperparameter tuning on the **training set only** (with [cross-validation](cross-validation.md)). The final evaluation must be done on a separate **test set** to avoid [overfitting](../foundations/overfitting-underfitting.md).
...
- Less common in [scikit-learn](../packages/scikit-learn/README.md) but integral in frameworks like TensorFlow/PyTorch.
...
- **Grid Search**: good for small spaces, [baseline](../evaluation/baselines-and-error-analysis.md) comparisons.
...
- Beware of **[overfitting](../foundations/overfitting-underfitting.md) to [cross-validation](cross-validation.md)** when search spaces are very large.
