...
Adaptive Boosting (AdaBoost) is an ensemble method that combines multiple weak learners (often shallow decision trees) sequentially, where each new learner focuses on correcting the mistakes of the previous ones.
...
- `n_estimators` too large can lead to overfitting; tune carefully.
- A smaller `learning_rate` often requires more estimators but may improve generalization.
...
- Careful tuning of `n_estimators` and `learning_rate` is crucial to avoid overfitting.
