...
- `subsample < 1.0` introduces randomness (stochastic gradient boosting), reducing overfitting.
- Deep trees can lead to overfitting; tune `max_depth` carefully.
...
- Use **early stopping** (via `validation_fraction` + `n_iter_no_change`) to prevent overfitting.
...
- Subsampling makes the method more robust and reduces overfitting.
- Strong baseline model for both classification and regression tasks.
