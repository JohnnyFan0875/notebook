...
Categorical encoding transforms categorical features into numeric representations that machine learning models can use. scikit-learn and related libraries provide multiple encoding strategies suitable for different scenarios.
...
- **Risk of overfitting** if computed on full dataset (must be computed only on training data).
...
- Always apply encoding **within cross-validation folds** or training set only to avoid leakage.
