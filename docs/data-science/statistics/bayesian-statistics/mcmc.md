...
Key point: The essence of MCMC: We do not need to know the formula of the posterior distribution, we only need to be able to evaluate the "unnormalized posterior" (i.e. likelihood × prior). MCMC exploits this by building a Markov chain such that its steady-state distribution is equal to the target posterior distribution, and then sampling from it.
...
Tip: After a burn-in period (warm-up), the chain "forgets" its starting point and the samples are approximately from the true posterior. We discard the burn-in and use only the post-warm-up samples. The burn-in period (warm-up) is the initial process of letting the chain converge to the posterior distribution, and the samples in this period are not used.
...
Warning: Proposal width matters: Too narrow → slow exploration (high autocorrelation). Too wide → most proposals rejected (low acceptance rate). Target acceptance rate for MH is roughly 23–50%.

## Modern Samplers: HMC and NUTS
...
Tip: NUTS is the default sampler in PyMC and Stan. You rarely need to tune it manually — it adapts step size and path length during warm-up.
...
Warning: Always run at least 4 chains for R̂ to be meaningful. One chain will almost always look fine even when the sampler hasn't explored the full posterior.
...
| **PPC** | Posterior predictive checks | Simulated data matches observed |
...
## MCMC Is an Approximation Workflow

MCMC does not symbolically solve the posterior. It approximates the posterior with samples, which is why diagnostics are part of the result, not an optional appendix.
