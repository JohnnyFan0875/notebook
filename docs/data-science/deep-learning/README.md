...
`notebook_temp/output` 內的 deep learning 抽取其實非常完整，涵蓋入門、影像、文字、transformer、訓練效率與 LLM fine-tuning。原本 notebook 只留下 `pytorch/` 子資料夾，確實太像 package 筆記，不像教材章節；因此這裡已改成概念與任務導向的結構。
